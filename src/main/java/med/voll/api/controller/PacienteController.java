...
public class PacienteController {
    ...
    @GetMapping
    public ResponseEntity<Page<DatosListadoPaciente>> listadoPacientes(@PageableDefault( size = 10, page = 0, sort = {"nombre"}) Pageable paginacion){
        Page<DatosListadoPaciente> page = pacienteRepository.findByActivoTrue(paginacion).map(DatosListadoPaciente::new);
    ...
    public ResponseEntity<DatosRespuestaPaciente> retornarDatosPaciente(@PathVariable Long id){
        Paciente paciente = pacienteRepository.getReferenceById(id);
        var datos =  new DatosRespuestaPaciente(paciente);
